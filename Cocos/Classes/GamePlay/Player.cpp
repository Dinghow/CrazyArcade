...
Animation* Player::createDyingAnimation()
{
...
Animation* Player::createDeleteAnimation()
{
	CCSpriteFrameCache* deadFrameCache = CCSpriteFrameCache::sharedSpriteFrameCache();
	deadFrameCache->addSpriteFramesWithFile("res/Role/bazzi.plist", "res/Role/bazzi.png");
	Vector<SpriteFrame*> frameArray;
...
{
	auto dyingAnimation = createDyingAnimation();
	dyingAnimation->setDelayPerUnit(0.3f);
	dyingAnimation->setLoops(5);
	auto dyingAnimate = Animate::create(dyingAnimation);
	role->runAction(dyingAnimate);
}

void Player::roleDelete()
{
	auto deleteAnimation = createDeleteAnimation();
	deleteAnimation->setDelayPerUnit(0.3f);
	deleteAnimation->setLoops(1);
	auto deleteAnimate = Animate::create(deleteAnimation);
